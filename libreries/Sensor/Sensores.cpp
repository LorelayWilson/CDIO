...
void Sensores::medirLuz(){
  ...
  if(lectura <= oscuro) Serial.println("Es de noche o el sensor esta tapado."); 
  if(lectura >= saturacion) Serial.println("Completamente soleado.");
  if(lectura > oscuro && lectura < ((saturacion-oscuro)/2)) Serial.println("Bastante nublado.");
  if(lectura > ((saturacion-oscuro)/2) && lectura < saturacion) Serial.println("Poco nublado.");
